...
<head>
  ...
  app.get('/', (req, res) => {
    res.send(
      ...
      '  <style>' +
      '    /* ...existing CSS... */' +
      '    body { background: #f0f7ff; color: #1e293b; font-family: Segoe UI, Tahoma, Geneva, Verdana, sans-serif; }' +
      '    .model-badge { background: #93c5fd; color: #1d4ed8; padding: 0.25rem 0.75rem; border-radius: 20px; font-size: 0.85rem; font-weight: 500; margin-left: 1rem; }' +
      '    .chat-container { max-width: 600px; margin: 2rem auto; background: #fff; border-radius: 12px; box-shadow: 0 10px 25px rgba(0,0,0,0.07); padding: 2rem; }' +
      ...
      // Serve the full chat app with restored style and structure
      app.get('/', (req, res) => {
        res.send(
          '<!DOCTYPE html>' +
          '<html lang="en">' +
